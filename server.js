...
app.get('/team_stats', function(req, res) {
  var game1 = "select * from football_games;";
...
app.get('/player_info/select_player', function(req, res) {
  var player_choose = req.query.player_choice;
  var query = 'SELECT id, year, major, passing_yards, rushing_yards, receiving_yards, img_src FROM football_players WHERE id=' + player_choose + ';';
  var query1 = "select id, name from football_players;";
  var query2 = 'SELECT count(*) FROM football_games WHERE ' + player_choose + ' = ANY(players);';
  db.task('get-everything', task => {
    task.any(query),
    task.any(query1),
    task.any(query2)
